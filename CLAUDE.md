...
53. **Statistieken menu** (6 items: overzicht, zwakste meter, club vertrouwen, mental score, schaduw-ronde, GPS-trail)
54. **Zwakste Meter AI analyse** (Claude API analyseert slagdata per afstandsrange)
55. **Club Vertrouwensmeter** (score per club op basis van gespeelde slagen)
56. **Mental Score tracking** (emotie per hole + correlatie met prestatie)
57. **Schaduw-Ronde** (vergelijk met je beste ronde op dezelfde baan)
58. **GPS-trail** (looplijn per hole met slagpunten)
59. **Statistieken overzicht** (gemiddelden, trends, par-verdeling)
60. **Rondes opgeslagen in Supabase** (persistent per gebruiker, laden bij startup)
61. **Scorekaarten bewerkbaar** (scores aanpassen vanuit ronde historie)
62. **HCP snapshot per hole** (handicap snapshot opgeslagen bij hole afronden)
63. **Stableford per hole opgeslagen** (punten per hole bewaard in rondedata)
64. **SL label** in scorekaart (slagen per hole zichtbaar)
65. **Herberekening na wijziging** (Stableford punten automatisch herberekend bij score-edit)
66. **Opgeslagen feedback** (bevestiging na succesvol opslaan van wijzigingen)
67. **SL alleen in overzicht** (slagen-label alleen in rondenlijst, niet in detail scorekaart)
68. **Fix re-render na hole wijziging** (scherm ververst direct na score-edit in scorekaart)
69. **60s trilreminder** (vibratie als nieuwe hole niet gestart wordt na 60 seconden)
70. **Putts als slag** in scorekaart (putts meegeteld in slagen-weergave)
71. **Info knop laadt course details** (HomeScreen haalt baaninfo op uit database)
72. **Score volgt putts** (score past automatisch mee als putts wijzigt, minimum score = putts+1)
73. **Score = slagen + putts** (score automatisch berekend uit aantal slagen + putts)
74. **Par/SI in hole header** (par en stroke index zichtbaar in scorekaart hole header)
75. **SI en par opgeslagen per hole** (stroke index en par bewaard in rondedata bij hole afronden)
76. **Stableford correct herberekend** (herberekening bij score-edit gebruikt opgeslagen SI/par/playingHcp)
77. **Scorekaart kleuren** (0PT grijs, score oranje bij +1, rood bij +2 of meer boven par)
78. **Balpositie tracking** (L/M/R per slag voor fairway accuracy analyse)
79. **Afstand per slag in scorekaart** (geslagen afstand zichtbaar in hole detail)
80. **GPS trail per hole** (coördinaten opgeslagen per slag voor route-weergave na ronde)
81. **Balpositie t.o.v. vlag** (op green: positie relatief tot de pin weergeven)
82. **GPS trail berekend** (route per hole berekend vanuit tee + richting + afstand + balpositie L/M/R)
83. **Positie per slag bewerkbaar** (balpositie L/M/R aanpasbaar in wijzigen modal)
84. **Kaartknop altijd zichtbaar** (GPS trail kaart altijd beschikbaar, data automatisch geladen)
...
88. **Foto past in scherm** (scorekaart foto-tab: hole foto schaalt naar viewport, geen scroll meer, tee-lijn volledig zichtbaar)
...
102. **Hole overlay context-aware** tee-tap en BEGIN HOLE alleen bij eerste opening, daarna "Terug naar invoer"
